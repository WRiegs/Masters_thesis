'''
This script is used to automatically cluster and filter the redundancy of the input fasta file
and then create a validation set consisting of 250 of the low identity sequences.
'''
...
def main():
    ...
    args.num_samples = min(args.num_samples, len(mmseq_list_exp))
    
    low_identity_subset = random.sample(mmseq_list_exp, args.num_samples)

    print("Subsetting to: ", args.num_samples, " samples")
    
    print('num of experimental low id in mmseqs2 0.3 reduced set: ', len(mmseq_list_exp))
    
    with open(f'{args.out}/Low30_mmseq_ID_{args.num_samples}_exp_subset.txt', "w") as f:
        for i in low_identity_subset:
            f.write(i + "\n")
