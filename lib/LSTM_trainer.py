'''
This script trains a model on the Contrastive learning Squidly embeddings. 
...
class ProteinGRU(nn.Module):
    def __init__(self, embedding_dim, hidden_dim, output_dim, num_layers, dropout_rate):
        super(ProteinGRU, self).__init__()
        self.hidden_dim = hidden_dim
    ...
    def forward(self, x, lengths):
        ...
        output = self.hidden2out(gru_out)
        return output
...
#         self.targets = targets
#         # get the lengths of the tensors for sequence_lengths
#         self.sequence_lengths = [tensor.shape[0] for tensor in tensors]

#     def __len__(self):
#         return len(self.tensors)

#     def __getitem__(self, idx):
#         return self.tensors[idx], self.targets[idx], self.sequence_lengths[idx]


# def collate_fn(batch):
#     """
#     batch: list of tuples (input_tensor, target_tensor, sequence_length)
#     """
#     # Unpack batch
...
#     targets = [targets[i] for i in perm_idx]
