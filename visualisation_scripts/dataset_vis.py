'''
This script was used to generate all the PCAs in the study. There were more that you can explore.
'''
...
def gather_validation_set_data(df, output, dataset = 1, reproduction_run_dir="/scratch/project/squid/code_modular/reproduction_runs/small"):
    ...
    plt.savefig(os.path.join(output, f'dataset{dataset}_validation_set_EC_TX_pie_distribution.png'))
...
def main():
    ...
    plt.savefig(os.path.join(args.output, 'EC_TX_distribution.png'))
    ...
    plt.savefig(os.path.join(args.output, 'EC_TX.X_distribution.png'))
    ...
    plt.savefig(os.path.join(args.output, 'Active_site_AAs_distribution.png'))
